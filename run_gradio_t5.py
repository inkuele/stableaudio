...
# === Load local T5 tokenizer and encoder ===
from transformers import T5Tokenizer, T5EncoderModel
ENC_PATH = os.path.join(os.path.dirname(__file__), 'encoders', 't5')
tokenizer = T5Tokenizer.from_pretrained(ENC_PATH, local_files_only=True)
encoder   = T5EncoderModel.from_pretrained(ENC_PATH, local_files_only=True)
# =============================
...
def load_model(selection: str):
    ...
    model = m.to(device)
    # ─── Override the model's text-conditioning modules to use our local T5 ───
    if hasattr(model, "conditioner"):
        for cond in model.conditioner.conditioners.values():
            if hasattr(cond, "tokenizer"):
                cond.tokenizer = tokenizer
            if hasattr(cond, "encoder"):
                cond.encoder   = encoder
    # =============================================================================
...
def generate_audio(
    ...
    for i, prompt in enumerate(lines):
        ...
        # build and combine negative conditioning
        cond = cond_pos.copy()
